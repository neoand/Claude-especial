...
$(git log --name-only --oneline -10 2>/dev/null | grep -v "^[a-f0-9]" | sort | uniq | head -20 || echo "Nenhum arquivo modificado recentemente")
...
- Melhorias genéricas → Sincronizar de volta com template
- Específico do domínio → Apenas neste projeto
...
**Template:** Claude-especial (LLM-First template)
